...
class NaiveBayes:
    ...
    def estimate_class_priors(self, labels: torch.Tensor) -> Dict[int, torch.Tensor]:
        ...
        total_count = torch.tensor(sum(label_counts.values()))
    ...
    def estimate_conditional_probabilities(
        ...
        # Accumulate word counts for each class using vectorized operations
        class_word_counts[0] += torch.sum(features[labels == 0], dim=0)
        class_word_counts[1] += torch.sum(features[labels == 1], dim=0)

        for label in class_word_counts:
            total_count: int = class_word_counts[label].sum()
            class_word_counts[label] = (class_word_counts[label] + delta) / (
                total_count + delta * self.vocab_size
            )
    ...
    def estimate_class_posteriors(
        ...
        log_prior_probs: torch.Tensor = torch.log(
            torch.tensor(
                [
        ...
        log_likelihoods: torch.Tensor = torch.matmul(log_probs, feature)
        log_posteriors: torch.Tensor = log_likelihoods + log_prior_probs
    ...
    def predict_proba(self, feature: torch.Tensor) -> torch.Tensor:
        ...
        log_posteriors: torch.Tensor = self.estimate_class_posteriors(feature)
        posteriors: torch.Tensor = torch.exp(log_posteriors)
        return posteriors / torch.sum(posteriors)
